...
def preformat_OGB_PCQM4Mv2(dataset_dir, name):
    ...
    dataset = PygPCQM4Mv2Dataset(root=dataset_dir, pre_transform=lap_eig_transform)
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    dataset = PygPCQM4Mv2ContactDataset(dataset_dir, subset='530k', pre_transform=lap_eig_transform)
...
def preformat_Peptides(dataset_dir, name):
    ...
    if dataset_type == 'functional':
        dataset = PeptidesFunctionalDataset(dataset_dir, pre_transform=lap_eig_transform)  # XXXX-1
    elif dataset_type == 'structural':
        dataset = PeptidesStructuralDataset(dataset_dir, pre_transform=lap_eig_transform)
    s_dict = dataset.get_idx_split()
    dataset.split_idxs = [s_dict[s] for s in ['train', 'val', 'test']]
    return dataset
...
def preformat_VOCSuperpixels(dataset_dir, name, slic_compactness):
    ...
    dataset = join_dataset_splits(
        [VOCSuperpixels(root=dataset_dir, name=name,
                        ...
                        split=split, pre_transform=lap_eig_transform)
         for split in ['train', 'val', 'test']]
    )
