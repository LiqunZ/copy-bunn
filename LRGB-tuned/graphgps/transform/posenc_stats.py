...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    # Verify PE types.
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'RWSE', 'RWSEEdge', 'HKdiagSE',
    ...
    if 'RWSEEdge' in pe_types:
        kernel_param = cfg.posenc_RWSEEdge.kernel
        if len(kernel_param.times) == 0:
            raise ValueError("List of kernel times required for RWSE")
        self_enc, edge_enc, global_enc, global_idx = get_edge_rw_landing_probs(
            ksteps=kernel_param.times,
            edge_index=data.edge_index,
            num_nodes=N,
            num_global=cfg.posenc_RWSEEdge.num_global
        )
        data = data.to_heterogeneous(node_type_names=['V'], edge_type_names=[('V', 'E', 'V')])
        if data.num_nodes == 1:
            data.y = data['V'].y
            del data['V'].y

        data['V', 'E', 'V'].pestat_RWSESelf = self_enc
        data['V', 'E', 'V'].pestat_RWSEEdge = edge_enc
        data['V', 'Global', 'V'].edge_index = global_idx
        data['V', 'Global', 'V'].pestat_RWSEGlobal = global_enc
...
def get_edge_rw_landing_probs(ksteps, edge_index, edge_weight=None, num_nodes=None, num_global=4):
    """Compute Random Walk landing probabilities for given list of K steps.
    ...
    Args:
        ...
        num_nodes: (optional) Number of nodes in the graph
    Returns:
        3D Tensor with shape (len(ksteps), num_node, num_nodes) with RW landing probs
    """
    ...
    deg = scatter(edge_weight, src, dim=0, dim_size=num_nodes, reduce='sum')  # Out degrees.
    deg_inv = deg.pow(-1.)
    deg_inv.masked_fill_(deg_inv == float('inf'), 0)

    if edge_index.numel() == 0:
        P = edge_index.new_zeros((num_nodes, num_nodes), dtype=torch.float)
    else:
    ...
    if ksteps == list(range(min(ksteps), max(ksteps) + 1)):
        # Efficient way if ksteps are a consecutive sequence (most of the time the case)
        Pk = torch.eye(num_nodes, dtype=torch.float) #P.clone().detach().matrix_power(min(ksteps))
        P_list.append(Pk)
        for k in range(min(ksteps), max(ksteps) + 1):
            Pk = Pk @ P
            P_list.append(Pk)
        ...
        # Explicitly raising P to power k for each k \in ksteps.
        for k in ksteps:
            P_list.append(P.matrix_power(k))

    rw_landing = torch.stack(P_list, dim=2)

    self_idx = torch.arange(num_nodes)
    self_enc = rw_landing[self_idx, self_idx]
    edge_enc = rw_landing[src, dst]

    num_global = min(num_global, num_nodes)
    probs = torch.ones((num_nodes, num_nodes))
    probs[self_idx, self_idx] = 0.0

    global_idx = torch.argmax(rw_landing[:, :, -1].sum(dim=0))
    global_idx = global_idx.view(-1).tile(num_nodes)
    global_edge_idx = torch.stack([global_idx, self_idx], dim=0)

    #uniform_idx = torch.randint(num_nodes, (2, num_nodes * num_global))
    #global_edge_idx = torch.cat([global_edge_idx, uniform_idx], dim=1)

    global_edge_idx = torch.unique(global_edge_idx, dim=1)
    global_edge_idx = torch.cat([global_edge_idx, global_edge_idx[[1, 0]]], dim=1)

    global_enc = rw_landing[global_edge_idx[0], global_edge_idx[1]]
    return self_enc, edge_enc, global_enc, global_edge_idx
...
def get_heat_kernels_diag(evects, evals, kernel_times=[], space_dim=0):
    ...
    if len(kernel_times) > 0:
        ...
        for t in kernel_times:
            # sum_{i>0}(synthexp(-2 t lambda_i) * phi_{i, j} * phi_{i, j})
...
def get_heat_kernels(evects, evals, kernel_times=[]):
    ...
    if len(kernel_times) > 0:
        ...
        for t in kernel_times:
            # sum_{i>0}(synthexp(-2 t lambda_i) * phi_{i, j1, ...} * phi_{i, ..., j2})
            heat_kernels.append(
